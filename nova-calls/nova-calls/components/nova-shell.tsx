...
export function NovaHome() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden pb-28 text-white lg:h-screen lg:overflow-hidden lg:pb-0">
      <TopChrome />
      <main className="relative z-10 grid min-h-screen w-screen gap-6 px-4 pb-6 pt-0 lg:grid-cols-[232px_minmax(0,1fr)_640px] lg:px-7 lg:pt-8">
        <Sidebar />

        <section className="min-w-0 pt-4 lg:pt-20">
          <h1 className="mx-auto max-w-[760px] text-center text-[clamp(42px,8vw,83px)] font-black leading-[.92] tracking-[-.075em] lg:text-left">
          ...
          <section className="nova-glass relative mt-5 overflow-hidden rounded-[26px] border-cyan-200/40 p-5 shadow-[0_0_28px_rgba(34,211,238,.25)]">
            ...
            <textarea
              ...
              rows={3}
              className="relative z-10 min-h-[74px] w-full resize-none bg-transparent text-base font-bold text-white outline-none placeholder:text-slate-300/70"
            />
          ...
          <div className="-mx-4 mt-6 flex gap-3 overflow-x-auto px-4 pb-2">
            {callTypes.map((item) => (
              <button
                ...
                className={`shrink-0 rounded-full border px-5 py-2 text-sm font-black ${type === item ? 'border-indigo-300/50 bg-gradient-to-r from-blue-500/70 to-violet-500/70' : 'border-white/10 bg-white/10'}`}
              >
                {item === 'Decidere' ? '◈' : item === 'Capire' ? '⚭' : item === 'Feedback' ? '▱' : item === 'Trovare persone' ? '♙' : item === 'Fare ora' ? '☆' : '▣'} {item}
              </button>
            ))}
          </div>
...
function Sidebar() {
  return (
    <aside className="nova-glass fixed bottom-3 left-3 right-3 z-50 rounded-3xl p-2 lg:static lg:mt-[88px] lg:flex lg:min-h-0 lg:flex-col lg:rounded-[18px] lg:p-4">
      <nav className="grid grid-cols-5 gap-1.5 lg:grid-cols-1 lg:gap-2">
        {navItems.map(([icon, label, href], index) => (
          <Link key={label} href={href} className={`relative flex h-12 items-center justify-center gap-3 rounded-2xl text-sm font-black lg:justify-start lg:px-3 ${index === 0 ? 'bg-cyan-300/10 text-cyan-100 shadow-[inset_0_0_0_1px_rgba(34,211,238,.18)]' : 'text-white/80 hover:bg-white/10'} ${index > 4 ? 'hidden lg:flex' : ''}`}>
            <span className="text-xl drop-shadow-[0_0_8px_rgba(34,211,238,.5)]">{icon}</span><span className="hidden lg:inline">{label}</span>{label === 'Notifiche' && <span className="ml-auto hidden rounded-full bg-white/15 px-2 py-1 text-xs lg:inline">3</span>}
          </Link>
        ))}
      </nav>
      <Link href="/calls/new" className="fixed bottom-[84px] right-5 grid h-16 w-16 place-items-center rounded-3xl bg-gradient-to-br from-rose-400 via-violet-500 to-cyan-300 text-4xl font-bold shadow-[0_0_36px_rgba(34,211,238,.48)] lg:static lg:mt-auto lg:flex lg:h-[74px] lg:w-auto lg:justify-between lg:px-5 lg:text-left lg:text-xl">
        <span className="lg:hidden">+</span><span className="hidden lg:block">Apri<br />una Call</span><span className="hidden lg:block">✦</span>
      </Link>
      <div className="mt-5 hidden items-center gap-2 rounded-full bg-white/5 px-3 py-2 text-xs font-bold tracking-widest text-white/60 lg:flex"><span className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_12px_#34d399]" />NOVA Online</div>
    </aside>
  );
}
...
function FeaturedCall({ call }: { call: NovaCall }) {
  return (
    <article className="relative mt-4 min-h-[330px] overflow-hidden rounded-[30px] border border-violet-400/40 bg-[radial-gradient(circle_at_76%_45%,rgba(139,92,246,.42),transparent_22%),linear-gradient(135deg,#071027,#11114a_47%,#080b1c)] p-6 shadow-[0_0_42px_rgba(139,92,246,.35)]">
      ...
      <div className="relative z-10 flex min-h-[280px] flex-col">
        <div className="flex justify-between gap-3"><span className="rounded-full border border-white/10 bg-white/10 px-4 py-2 text-sm font-black">★ Call in evidenza</span><span className="rounded-full border border-white/10 bg-white/10 px-4 py-2 text-sm font-black"><span className="mr-2 inline-block h-2 w-2 rounded-full bg-emerald-400" />In corso</span></div>
        <h2 className="mt-10 max-w-lg text-4xl font-black leading-none tracking-[-.045em]">{call.title}</h2>
        <p className="mt-3 max-w-xl text-base font-semibold text-slate-300">{call.description}</p>
...
function HostCard() {
  return (
    <section className="nova-glass mt-4 grid gap-5 rounded-[22px] bg-gradient-to-r from-slate-950/80 to-violet-950/60 p-5 md:grid-cols-[1fr_1.2fr] md:items-center">
      <div className="flex items-center gap-5"><div className="h-20 w-20 rounded-full bg-[radial-gradient(circle_at_34%_25%,#a5f3fc,#a855f7_32%,#db2777_54%,#020617_78%)]" /><div><span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs font-black">✦ Host</span><div className="mt-2 text-2xl font-black">Giulia R. <span className="rounded-full bg-white/10 px-2 py-1 text-[10px]">Host</span></div><div className="text-sm font-bold text-slate-300">Designer • Milano</div><p className="mt-1 max-w-xs text-xs font-semibold text-white/65">Creator di spazi che aiutano le persone a prendere decisioni migliori.</p></div></div>
      <div className="grid grid-cols-3 border-t border-white/10 pt-4 text-center md:border-l md:border-t-0 md:pt-0"><Metric label="Call ospitate" value="47" /><Metric label="Outcome generati" value="23" /><Metric label="Persone aiutate" value="1.2K" /></div>
    </section>
  );
}

function Metric({ label, value }: { label: string; value: string }) { return <div><span className="block text-xs font-bold text-white/50">{label}</span><b className="mt-2 block text-2xl text-cyan-300">{value}</b></div>; }

function LiveStrip({ calls }: { calls: NovaCall[] }) {
  return <section className="mt-4 rounded-[22px] border border-violet-400/20 bg-slate-950/45 p-4"><div className="mb-3 flex items-center justify-between"><h3 className="text-xl font-black">⌁ Live adesso</h3><span className="text-xs font-black text-slate-300">Vedi tutte →</span></div><div className="flex gap-3 overflow-x-auto pb-1">{calls.slice(0,6).map((call, index) => <Link href={`/c/${call.slug}`} key={`${call.slug}-${index}`} className="relative min-h-[86px] w-[150px] shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-cyan-400/20 to-slate-950/70 p-3 text-sm font-black"><span className="mb-2 block text-[10px] text-lime-300">● In corso</span>{call.title}<span className="absolute bottom-2 left-3 text-[11px] text-lime-300">↯ {call.pulse}</span></Link>)}</div></section>;
}
